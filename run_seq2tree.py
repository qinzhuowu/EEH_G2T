...
if dataset=="APE":
    ...
    group_data = read_json("data/mawps/new_MAWPS_processed.json")
else:
    # 加载数据集
...
if dataset=="mawps":
    ...
    fold_size1 = int(len(pairs) * 0.8)
    fold_size2 = int(len(pairs) * 0.9)
    pairs_from_valid = pairs[fold_size1:fold_size2]
    pairs_from_test = pairs[fold_size2:]
    pairs = pairs[:fold_size1]
else:
...
if dataset=="mawps":
    input_lang,output_lang,train_pairs,test_pairs,matrix,category_vocab,hownet_dict_vocab=prepare_english_data(pairs, pairs_from_test,
    5, generate_nums,copy_nums, tree=True)
    valid_pairs=prepare_english_valid_data(input_lang,output_lang,pairs_from_valid, tree=True)
else:
        ...
        for i in range(len(input_length)):
            for j1 in range(input_length[i]):
                for j2 in range(input_length[i]):
                    ...
                    if dataset=="mawps":
                        if word2==word1 and word2 not in punc_list:
