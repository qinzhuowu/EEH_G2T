...
def load_Math23K_data(filename):  # load the json data to list(dict()) for MATH 23K
    ...
    for i, s in enumerate(f):
        ...
        if i % 7 == 0:  # every 7 line is a json
            ...
            question, equation, answer,id_ = data_d['segmented_text'].strip(), data_d['equation'], data_d['ans']
...
def load_data_withgroup(filename,group_file):
    D = []
    print(filename)
    question_num=0
    not_equal=0
    ...
    for l in open(filename):
        ...
        if question_num%divide==0:
            ...
            try:
                if is_equal(eval(equation), eval(answer)):
                    D.append((question, equation, answer,g_group))
                else:
                    #print("not equal")
                    #print(question)
                    #print(equation)
...
def load_Math23K_data_withgroup(filename,group_file):  # load the json data to list(dict()) for MATH 23K
    ...
    with open("data/Math23K_edge.json","r") as f2:
        edge=json.load(f2)
    ...
    for d in data:
        ...
        if len(out_seq) >0:
            if len(out_seq)> Max_Expression_len:
                ...
                if USE_just_char_number==True:
                    pairs.append((realnum_input, out_seq, nums, realnum_pos,group,depen))
                else:
                    pairs.append((input_seq, out_seq, nums, num_pos,group,depen))
    print("count_empty")
    print(count_empty)
        ...
        if word1 in hownet_dict_vocab:
            ...
            if len(cate1) >0:
                for j in range(len(input_list)):
                    word2= input_list[j]
                    #if word2 in word1
                    if word2==word1 and len(word1)>1 and word1!="NUM":
                        input_edge[i][j]=1
                        input_edge[j][i]=1
                    '''
...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_trained:
        ...
        if USE_KAS2T_encoder==False:
            ...
            category_match_list=[]
            train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
                pair[2], pair[3], num_stack,category_index_list,category_match_list,middle_exp_cell,depen_list))
        else:
...
def prepare_english_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
...
def prepare_english_valid_data(input_lang, output_lang,pairs_tested, tree=False):
    test_pairs=[]
    for pair in pairs_tested:
        num_stack = []
        for word in pair[1]:
...
def prepare_train_batch(pairs_to_batch, batch_size,PAD_token):
    ...
    for batch in batches:
        ...
        dependency_batch=[]
        if USE_KAS2T_encoder==False:
            for i, li, j, lj, num, num_pos, num_stack,_,_,_,depen in batch:
                ...
                dependency_batch.append(depen)
        else:
            for i, li, j, lj, num, num_pos, num_stack,unit_list,rule3_list,middle_exp_cell,depen in batch:
                num_batch.append(num)
                input_batch.append(pad_seq(i, li, input_len_max))
                output_batch.append(pad_seq(j, lj, output_len_max))
                num_stack_batch.append(num_stack)
        ...
        dependency_batches.append(dependency_batch)
        if USE_KAS2T_encoder==True:
    ...
    return input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches,unit_list_batches,rule3_list_batches,output_middle_batches,dependency_batches
