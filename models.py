...
class EncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size,pos_embedding_size, hidden_size,category_size,pretrain_emb,edge_size, n_layers=2, dropout=0.5):
        ...
        if USE_KAS2T_encoder==False:
            ...
            self.dis_embedding1 = Dis_PositionalEncoding(384, 160)
            #self.dis_embedding = Dis_PositionalEncoding(384, 160)
    ...
    def forward(self, input_seqs, input_lengths,cate_word_edge,cate_index_input,cate_length,cate_id_match,nums_index,num_pos,batch_graph,edge_graph, hidden=None):
        ...
        if USE_KAS2T_encoder==False:
            ...
            pade_outputs = pade_outputs[:, :, :384] + pade_outputs[:, :,384:]  # S x B x H
            ...
            if max_cate_len>0:
                ...
                for idx in range(len(cate_length)):
                    curr_seq_len=input_lengths[idx]
                    temp_hidden_category=[]
                    for i in range(curr_seq_len):
                        #print("pade_outputs[i][idx]")
                        #print(pade_outputs[i][idx].size())
                        temp_hidden_category.append(pade_outputs[i][idx])#hidden

                    idx_cate_len=cate_length[idx]
                    if idx_cate_len == 0:
                        for cate_idx in range(max_cate_len):
                            temp_hidden_category.append(padding_hidden)
                        #temp_hidden_category.append(torch.stack([padding_hidden for cate_idx in range(max_cate_len)],dim=0))#C*H
                    else:
                        temp_hidden=[]
                        for i in range(idx_cate_len):
                            for j in cate_id_match[idx][i]:
                                temp_hidden.append(pade_outputs[j][idx])#*catehidden
                            gather_hidden=torch.stack(temp_hidden,dim=0).mean(0)#hidden
                            #print("gather_hidden")
                            #print(gather_hidden.size())
                            temp_hidden_category.append(gather_hidden)
                        for i in range(idx_cate_len,max_cate_len):
                            #print(padding_hidden.size())
                            temp_hidden_category.append(padding_hidden)
                    for i in range(curr_seq_len,seq_len):
                        temp_hidden_category.append(padding_hidden)
                    temp_hidden_category=torch.stack(temp_hidden_category,0)
                    category_embedded_temp.append(temp_hidden_category)#(S+C)*hidden

                concat_input_category = torch.stack(category_embedded_temp,0)#B*S*H

                #concat_input_category=torch.cat((pade_outputs.transpose(0,1),category_embedded),1)#B*S+cate_num*hidden
            else:
                concat_input_category=pade_outputs.transpose(0,1)#B*S*N
            
            input_sequence=input_seqs.transpose(0,1)

            
            seq_len=input_seqs.size(0)
            node_embed=self.edge_dense1(pade_outputs.transpose(0,1))
            h1_embed=node_embed.unsqueeze(2).repeat([1,1,seq_len,1])#B*S*S*H
            h2_embed=node_embed.unsqueeze(1).repeat([1,seq_len,1,1])#B*S*S*H
            edge_mask=edge_graph.unsqueeze(3).repeat([1,1,1,32])
            h1_embed=h1_embed.masked_fill_(edge_mask==0,0)
            h2_embed=h2_embed.masked_fill_(edge_mask==0,0)

            edge_embed=self.edge_embedding(edge_graph)#B*S*S*E
            edge_embed_cat=torch.cat((edge_embed,h1_embed,h2_embed),3)

            edge_graph_expend=edge_embed_cat.permute(0,3,1,2) #B*H*S*S
            ZeroPad = nn.ZeroPad2d(padding=(0, max_cate_len, 0, max_cate_len))#B*1*S+c*S+C
            edge_graph_expend = ZeroPad(edge_graph_expend).permute(0,2,3,1)#B*H*S*S
            #dis_graph=batch_graph[:,3,:,:]
            #dis_graph=dis_graph.long()
            #dis_embed1=self.dis_embedding(dis_graph,max_cate_len)#B*S*S
            
            #dis_embed2=self.dis_embedding1(cate_word_edge_dis,0)#B*S*S
            #print(dis_embed.size())
            encoder_outputs_knowledge = torch.stack([att(concat_input_category,cate_word_edge,edge_graph_expend) for att in self.gat_n], dim=2)#B*S*N*head
...
class Prediction(nn.Module):
    ...
    def forward(self, node_stacks, left_childs, encoder_outputs, num_pades, padding_hidden, seq_mask, mask_nums,indices,masked_index,question_mask):
        ...
        #sen_attn3=self.attn3(current_embeddings.transpose(0, 1), encoder_outputs, question_mask[:,:,2])
        #question_context3=sen_attn3.bmm(encoder_outputs.transpose(0, 1))  # B x 1 x N
        
        #sen_attn4=self.attn4(current_embeddings.transpose(0, 1), encoder_outputs, question_mask[:,:,3])
        #question_context4=sen_attn4.bmm(encoder_outputs.transpose(0, 1))  # B x 1 x N
        #sen_attn5=self.attn5(current_embeddings.transpose(0, 1), encoder_outputs, question_mask[:,:,4])
        #question_context5=sen_attn5.bmm(encoder_outputs.transpose(0, 1))  # B x 1 x N
...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        self.a3 = nn.Parameter(torch.zeros(size=(96, 1)))
        nn.init.xavier_uniform_(self.a1.data, gain=1.414)
        nn.init.xavier_uniform_(self.a2.data, gain=1.414)
        nn.init.xavier_uniform_(self.a3.data, gain=1.414)
        self.leakyrelu = nn.LeakyReLU(self.alpha)

    def forward(self, graph_input, adj,edge_embed):
        #[B*S*H] [B*S*S]
        ...
        middle_result_dis1=torch.matmul(edge_embed, self.a3).squeeze(3)#B*S*S*N
        e = self.leakyrelu(middle_result1 + middle_result2+middle_result_dis1)
...
class Dis_PositionalEncoding(nn.Module):
    
    def __init__(self, d_model, max_seq_len):
        """ d_model hidden 512  max_seq_len largedt len
        """
        super(Dis_PositionalEncoding, self).__init__()
        # PE matrix
        position_encoding = np.array([
          [pos / pow(10000, 2.0 * (j // 2) / d_model) for j in range(d_model)]  for pos in range(max_seq_len)])
        # odd line use sin,even line use cos
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    #scores=scores+dis_embed.permute(0,3,1,2).contiguous()
    if mask is not None:
...
class MultiHeadAttention(nn.Module):
    def __init__(self, model_dim=512, num_heads=8, dropout=0.1):
        ...
        self.linear_dis=nn.Linear(384, num_heads)
        self.attn = None
    ...
    def forward(self, query, key, value, mask=None):
        ...
        #dis_embed_dense=self.linear_dis(dis_embed)
